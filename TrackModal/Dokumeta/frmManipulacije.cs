...
namespace TrackModal.Dokumeta
{
    public partial class frmManipulacije : Form
    {
        ...
        public frmManipulacije(string Korisnik, int PrijemnicaOtpremnica, int Vozom, int IzPrijema)
        {
            if (IzPrijema == 0)
            {
                IzPrijemnice = 1; //Podatak dosao iz Prijema
            }
            else
            {
                IzPrijemnice = 0; // Podatak Dosao iz Otpreme
            }
        ...
        private void frmManipulacije_Load(object sender, EventArgs e)
        {
            ...
            if (IzPrijemnice == 1)
            {
                ...
                else
                {
                    ...
                    " FROM [dbo].[PrijemKontejneraVoz]  where Vozom = 0 and PrijemnicaKontejneraVoz = "  + pomPrijemnica + "   order by  PrijemKontejneraVoz.[DatumPrijema] desc, PrijemKontejneraVoz.[ID] ";
...
" FROM [dbo].[OtpremaKontejnera]    inner join Voz on Voz.ID = OtpremaKontejnera.IdVoza  where OtpremaKontejneraVoz.[ID] = " + pomPrijemnica + " otpremakontejnera.NacinOtpreme = 1 order by OtpremaKontejnera.[DatumOtpreme] desc, OtpremaKontejnera.[ID]";
                    var s_connection = ConfigurationManager.ConnectionStrings["WindowsFormsApplication1.Properties.Settings.NedraConnectionString"].ConnectionString;
                    SqlConnection myConnection = new SqlConnection(s_connection);
                    var c = new SqlConnection(s_connection);
                    var dataAdapter = new SqlDataAdapter(select, c);
                ...
                else
                {
                   ...
                   " CONVERT(varchar, OtpremaKontejnera.[DatumOtpreme], 104) + ' ' + " +
                   " SUBSTRING(CONVERT(varchar, OtpremaKontejnera.[DatumOtpreme], 108), 1, 5))) as Naziv " +
                   " FROM[dbo].[OtpremaKontejnera]     where OtpremaKontejneraVoz.[ID] = " + pomPrijemnica + " otpremakontejnera.NacinOtpreme = 0 order by OtpremaKontejnera.[DatumOtpreme] desc, OtpremaKontejnera.[ID]";
